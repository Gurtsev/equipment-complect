...
create or replace function public.set_project_history_user_id()
...
drop trigger if exists set_project_history_user_id on public.project_history;
create trigger set_project_history_user_id
  before insert on public.project_history
  for each row execute function public.set_project_history_user_id();

alter table public.project_history enable row level security;

create policy "ph_select" on public.project_history
  for select to authenticated using (true);

create policy "ph_insert" on public.project_history
