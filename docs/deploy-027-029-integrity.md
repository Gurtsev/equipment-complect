# Применение миграций 027–029: целостность операций

> Production: миграции применены 2026-08-18 по подтверждению владельца проекта.
...
3. `029_atomic_custody_operations.sql`.

Frontend совместим с состоянием до миграций: если RPC из 029 отсутствуют в schema
cache, используются прежние запросы. Серверная защита от конкурентных операций и
атомарность истории появляются только после применения миграций.
...
where routine_schema = 'inventory'
  and routine_name in (
    ...
    'create_equipment_loan',
    'return_equipment_loan'
  )
order by routine_name;
```

Первые два запроса должны вернуть по две строки, последний — четыре. Затем проверить через интерфейс:
...
5. у каждой успешной выдачи/возврата одновременно появляется ровно одна запись истории.
