...
- ✅ Чистый передеплой в схему `inventory`; актуальный bootstrap генерируется как `combined_001_XXX.sql`
...
- [x] **3. Переехали все таблицы из `public` → схему `inventory`** — чистый передеплой выполнен; актуальный bootstrap: `combined_001_XXX.sql`
...
- [x] Чистый передеплой выполнен; актуальный bootstrap схемы: `combined_001_XXX.sql`
